alter table public.profiles
  add column if not exists pricing_mode text check (pricing_mode in ('rate', 'package')),
  add column if not exists pricing_session_track text check (pricing_session_track in ('mentoring', 'course')),
  add column if not exists pricing_package_id uuid references public.commercial_packages(id) on delete set null;
